...
TEST(algorithms_sorting, merge_sort_empty) {
    std::vector<int> data{};
    EXPECT_FALSE(wingmann::algorithms::sorting::merge_sort(data));
}
...
TEST(algorithms_sorting, merge_sort_two_values) {
    ...
    for (std::size_t test_count = 0; test_count < 10; ++test_count) {
        ...
        auto sorted = wingmann::algorithms::sorting::merge_sort(data);
        EXPECT_TRUE(sorted);
...
TEST(algorithms_sorting, merge_sort_several_values_descending) {
    ...
    for (std::size_t test_count = 0; test_count < 10; ++test_count) {
        ...
        auto sorted = merge_sort(data, wingmann::algorithms::sorting::sorting_type::descending);
        EXPECT_TRUE(sorted);
