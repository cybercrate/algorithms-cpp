...
TEST(algorithms_search, linear_search_empty) {
    EXPECT_EQ(std::nullopt, wingmann::algorithms::search::linear_search(std::vector<int>{}, 1));
}

TEST(algorithms_search, linear_search_one_value) {
    EXPECT_EQ(0, wingmann::algorithms::search::linear_search(std::vector<int>{64}, 64).value());
}
...
TEST(algorithms_search, linear_search_several_values) {
    ...
    for (std::size_t test_count = 0; test_count < 10; ++test_count) {
        data = wingmann::utility::test_helpers::get_sorted_vector_with_random_values<int>(100);
        target = wingmann::utility::test_helpers::get_value<int>(data);
        searched_target = data[wingmann::algorithms::search::linear_search(data, target).value()];
...
TEST(algorithms_search, linear_search_not_found) {
    ...
    for (std::size_t test_count = 0; test_count < 10; ++test_count) {
        ...
        EXPECT_EQ(std::nullopt, wingmann::algorithms::search::linear_search(data, target));
    }
}
