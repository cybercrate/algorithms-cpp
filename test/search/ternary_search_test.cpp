...
TEST(algorithms_search, ternary_search_empty) {
    EXPECT_EQ(std::nullopt, wingmann::algorithms::search::ternary_search(std::vector<int>{}, 1));
}

TEST(algorithms_search, ternary_search_one_value) {
    EXPECT_EQ(0, wingmann::algorithms::search::ternary_search(std::vector<int>{64}, 64).value());
}
...
TEST(algorithms_search, ternary_search_several_values) {
    ...
    for (std::size_t test_count = 0; test_count < 10; ++test_count) {
        ...
        searched_target = data[wingmann::algorithms::search::ternary_search(data, target).value()];
...
TEST(algorithms_search, ternary_search_not_found) {
    ...
    for (std::size_t test_count = 0; test_count < 10; ++test_count) {
        data = wingmann::utility::test_helpers::get_sorted_vector_with_random_values<int>();
        EXPECT_EQ(std::nullopt, wingmann::algorithms::search::ternary_search(data, target));
    }
}
